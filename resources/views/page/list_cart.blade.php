                        ...
                        @foreach(Session::get('Cart')->items as $item)
                            <tr class="cart_item">
                                ...
                                <td class="product-subtotal">
                                    <span class="amount">{{number_format($item['item']->unit_price * $item['qty'])}} đ</span>
                                </td>
                                <td class="product-update">
                                    <a href="javascript:" class="remove" title="Update this item"><i onclick="updatecartitem({{$item['item']->id}})" class="fa fa-refresh" data-id="{{$item['item']->id}}" id="update-cart-{{$item['item']->id}}"></i></a>
                                </td>
                                <td class="product-remove">
                                    <a href="javascript:" class="remove" title="Remove this item"><i onclick="delcartitem({{$item['item']->id}})" class="fa fa-trash-o" data-id="{{$item['item']->id}}" ></i></a>
                                </td>
                            </tr>
                        @endforeach
                        </tbody>
...
<script>
    ...
    function updatecartitem(id) {
        console.log(id);
        const quantity = jQuery("#qty-item-"+id).val();
        jQuery.ajax({
    ...
    function updatecart() {
        ...
        jQuery("table tbody tr td").each(function () {
            jQuery(this).find("input").each(function () {
                const element = {key: jQuery(this).data("id"), value: jQuery(this).val()};
                list_id.push(element);
            });
        });
        jQuery.ajax({
            url:'update-all-cart',
    ...
    function RenderCart(response) {
        jQuery("#change-cart").empty();
        jQuery("#change-cart").html(response);
